...
(def py-path (files/join plugins/*plugin-dir* "fs-src/ltfsclient/ltfsclient.fsx"))
...
(defn check-fsharp [obj]
  (assoc obj :fsharp (get-fsi)))

(defn check-client [obj]
  (assoc obj :fsharp-client (files/exists? py-path)))
...
(defn notify [obj]
  (let [{:keys [fsharp project-path path fsharp-client client]} obj]
    (cond
     ...
     (not project-path) (do
                          ...
                          (popup/popup! {:header "We couldn't find this file."
                                       :body "In order to evaluate in F# files, the file has to be on disk somewhere."
...
(defn try-connect [{:keys [info]}]
  (let [path (:path info)
        client (clients/client! :fsharp.client)]
    (check-all {:path path
                :client client})
    client))

(defn fsharp-watch [meta src]
...
(behavior ::watch-src
                  ...
                  :reaction (fn [editor cur meta src]
                              (fsharp-watch meta src)))

(behavior ::on-eval
                  :triggers #{:eval}
                  :reaction (fn [editor]
                              (object/raise fsharp :eval! {:origin editor
                                                             :info (assoc (@editor :info)
                                                                     :code (watches/watched-range editor nil nil fsharp-watch))})))

(behavior ::on-eval.one
                  :triggers #{:eval.one}
                  :reaction (fn [editor]
                              (let [code (watches/watched-range editor nil nil fsharp-watch)
...
(behavior ::fsharp-watch
                  :triggers #{:editor.eval.fsharp.watch}
...
(behavior ::fsharp-result
                  :triggers #{:editor.eval.fsharp.result}
                  :reaction (fn [editor res]
                              (notifos/done-working)
                              (object/raise editor :editor.result (:result res) {:line (:end (:meta res))
                                                                                 :start-line (-> res :meta :start)})))

(behavior ::fsharp-success
                  :triggers #{:editor.eval.fsharp.success}
                  :reaction (fn [editor res]
                              (notifos/done-working)
                              (object/raise editor :editor.result "✓" {:line (-> res :meta :end)
                                                                       :start-line (-> res :meta :start)})))

(behavior ::fsharp-exception
                  :triggers #{:editor.eval.fsharp.exception}
...
(behavior ::fsharp-image
                  :triggers #{:editor.eval.fsharp.image}
...
(behavior ::fsharp-printer
                  :triggers #{:editor.eval.fsharp.print}
...
(behavior ::eval!
                  ...
                  :reaction (fn [this event]
                              (let [{:keys [info origin]} event
                                ...
                                (notifos/working "")
                                (clients/send (eval/get-client! {:command :editor.eval.fsharp
                                                                 :origin origin
                                                                 :info info
                                                                 :create try-connect})
                                              :editor.eval.fsharp
...
(object/object* ::fsharp-lang
                :tags #{:fsharp.lang})

(def fsharp (object/create ::fsharp-lang))

(scl/add-connector {:name "F#"
                    :desc "Select a directory to serve as the root of your F# project."
                    :connect (fn []
                               (dialogs/dir fsharp :connect))})

(behavior ::fsharp-exe
                  :triggers #{:object.instant}
                  :desc "F#: Set the path to the F# executable for clients"
                  :type :user
                  :params [{:label "path"
                            :type :path}]
                  :exclusive true
                  :reaction (fn [this exe]
                              (object/merge! fsharp {:fsharp-exe exe})))

(behavior ::ifsharp-exe
                  :triggers #{:object.instant}
                  :desc "F#: Set the path to ifsharp for clients"
                  :type :user
                  :exclusive true
                  :params [{:label "path"
                            :type :path}]
                  :reaction (fn [this exe]
                              (object/merge! fsharp {:ifsharp-exe exe})))
